...
from yolink.exception import YoLinkClientError
...
async def async_setup_entry(hass: HomeAssistant, entry: YoLinkConfigEntry) -> bool:
    """Set up yolink_local from a config entry."""
    session = aiohttp_client.async_get_clientsession(hass)

    yolink_home = YoLinkLocalHubClient(
        session,
    ...
    try:
        async with asyncio.timeout(10):
            await yolink_home.async_setup(YoLinkHomeMessageListener(hass, entry))
    except (YoLinkClientError, ClientError, TimeoutError) as err:
